<?php
if(!class_exists("CvioBase")) {
	class CvioBase {
    	public $key = "37CEAC4DD405F251";
    	private $product_id = "8";
    	private $product_base = "cvio";
    	private $server_host = "https://bslthemes.com/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_cvio_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=cvio_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
